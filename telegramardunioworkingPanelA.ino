...
int receivedValueC = stopGap;

byte mac[] = {
  0x90, 0xA2, 0xDA, 0x0E, 0xC0, 0xA9
};
...
//Blue 0x0FC92
//Purple 0x6603A0
long int colorPrimary = 0x0FC92;

unsigned int listeningPort = 12111;

IPAddress panelBDestIP(141, 117, 45, 113);
int panelBPort = 12113;
long int colorSecondary = 0xF06F00;

IPAddress panelCDestIP(141, 117, 45, 114);
int panelCPort = 12112;
long int colorTertiary = 0x6603A0;

EthernetUDP UDP;
OscUDP oscUdp;
NetAddress panelBDest;
NetAddress panelCDest;
long netTimerEnd;
...
void setup() {
  ...
  panelCDest.set(panelCDestIP, panelCPort);
...
void panelBRead(OscMessage & msg) {
  ...
  if (data != stopGap)
  {
    if (receivedValueB != data)
    {
      ...
      //Serial.println(data);
      receivedValueB = data;
      leds[receivedValueB] = colorSecondary;
      FastLED.show();
...
void sendOscNet() {
  if (sendValue != stopGap)
  {
    OscMessage msgPanelB("/panelA");      //identify ourselves as panel C
    msgPanelB.add(sendValue);             //attach the value we want to send
    oscUdp.send(msgPanelB, panelBDest);   //send to panel A

    OscMessage msgPanelC("/panelA");      //identify ourselves as panel C
    msgPanelC.add(sendValue);             //attach the value we want to send
    oscUdp.send(msgPanelC, panelCDest);   //send to value B
  }
  sendValue = stopGap;
}
